...
<p><a href='iframe-Request-Report-ITE.php'>Back</a>
<p>Selected rows:</p>
...
<table id="myTable" style="font-size: 12px">
			<thead>
              ...
              $result = mysqli_query($link,"SELECT * FROM request WHERE date(D_n_T) BETWEEN '".$startdate."' AND '".$enddate."' AND UnitType REGEXP '[[:<:]]".$unittype."[[:>:]]' AND property_no = '".$property_no."'");   

              echo "Dates Between: $startdate and $enddate";
              echo "Unit Type: $unittype";
              echo "Property Number: $property_no";
              ?>
...
function generate() {
  ...
  const header = function(data) {
    ...
    doc.text('Equipment History Report',  data.settings.margin.left + 325, 60 )
  };
  ...
  doc.save('ITEquipmentHistoryReport'+d+'.pdf');
}
  </script>
</body>
</html>
